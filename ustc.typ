...
#let showybox-frame-style = (
  title-color: secondary-color.lighten(8%),
  body-color: secondary-color.transparentize(88%),
...
#let new-section-slide(level: 1, numbered: true, body, ..args) = touying-slide-wrapper(self => {
  let info = self.info + args.named()
  let header(self) = {
    set align(top)
    grid(
      ...
      block(
        ...
        //   text(fill: self.colors.primary, weight: "bold", size: 1.2em, ""),
        //   // text(fill: self.colors.primary.lighten(65%), utils.call-or-display(self, self.store.header-right)),
        // ),
      ),
    )
...
#let university-theme(
  ...
  show: touying-slides.with(
    config-page(
      ...
      header-ascent: 0em,
      footer-descent: 0em,
      margin: (top: 3em, bottom: 1.25em, x: 2em),
    ),
    config-common(
      slide-fn: slide,
...
// #let tertiary-color = blue.darken(30%)
// #let text-color = black.transparentize(20%)
      primary: blue.darken(60%),
      secondary: blue.darken(40%),
      tertiary: blue.darken(30%),
      neutral-lightest: rgb("#ffffff"),
